...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=200,
        ...
        condition_guidance_w=0.1, ar_inv=True, train_only_inv=False):
        super().__init__()
        ...
        if self.ar_inv:
            self.inv_model = ARInvModel(hidden_dim=hidden_dim, observation_dim=observation_dim, action_dim=action_dim).to(device='cuda')
        else:
        ...
        alphas = 1. - betas
        alphas_cumprod = torch.cumprod(alphas, axis=0).to(device='cuda')
        ...
        # calculations for diffusion q(x_t | x_{t-1}) and others
        self.register_buffer('sqrt_alphas_cumprod', torch.sqrt(alphas_cumprod).to(device='cuda'))
        self.register_buffer('sqrt_one_minus_alphas_cumprod', torch.sqrt(1. - alphas_cumprod).to(device='cuda'))
        self.register_buffer('log_one_minus_alphas_cumprod', torch.log(1. - alphas_cumprod).to(device='cuda'))
        self.register_buffer('sqrt_recip_alphas_cumprod', torch.sqrt(1. / alphas_cumprod).to(device='cuda'))
        self.register_buffer('sqrt_recipm1_alphas_cumprod', torch.sqrt(1. / alphas_cumprod - 1).to(device='cuda'))
    ...
    def get_loss_weights(self, discount):
        ...
        dim_weights = torch.ones(self.observation_dim, dtype=torch.float32).to(device='cuda')

        ## decay loss with trajectory timestep: discount**t
        discounts = discount ** torch.arange(self.horizon, dtype=torch.float).to(device='cuda')
        discounts = discounts / discounts.mean()
        loss_weights = torch.einsum('h,t->ht', discounts, dim_weights)
